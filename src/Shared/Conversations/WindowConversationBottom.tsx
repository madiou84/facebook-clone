...
import { VideoCameraIcon, PhoneIcon, MinusIcon, XIcon, PlusCircleIcon, HeartIcon } from '@heroicons/react/solid';

import { classNames } from '../helper';
import { FbIcon, Message, MessageToItem, MessageFromItem, ImageProfileUser } from '../index';

export default function WindowConversationBottom ({ user, positionRight = 'right-20', onClickMinus, onClickClose, ...rest }: any) {
    return (
        <div
            {...rest}
            className={classNames('hidden xl:block fixed bottom-0 z-50', positionRight)}
        >
            <div style={{ width: '20rem' }} className="h-auto bg-white rounded-t-md shadow-2xl">
                <div className="flex flex-col justify-between">
                
                    <div className="space-x-2 px-2 w-full flex items-center justify-between h-12 border-b border-gray-100 shadow">
                        {user.src && <div className="w-10 h-10">
                            <ImageProfileUser
                                src={user.src}
                                hasStory={false}
                                widthClass="w-full"
                        ...
                        <div className="w-full h-full flex items-center justify-between">
                            ...
                            <div className="flex items-center justify-center space-x-4">
                                <button type="button" className="focus:outline-none" onClick={onClickMinus}>
                                    <MinusIcon className="w-5 h-5 text-gray-400"/>
                                </button>
                                
                                <button type="button" className="focus:outline-none" onClick={onClickClose}>
